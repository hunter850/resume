...
function ListenScroll(props) {
    ...
    const shouldEnterBothSides = useShouldEnterBothSides(childRef, offset, backOffset, mode);
    ...
    return React.Children.map(children, (Child) => {
        ...
        if (typeof Child.type === "function") {
            return React.Children.map(children.type(), (child) => {
                const childClass = child.props && child.props.className ? child.props.className + " " : "";
                return React.cloneElement(child, {
                    className: (() => {
                        // 子層的start end 優先級較高
                        if (isIn) {
                            return child.props.end ? childClass + child.props.end : childClass + end;
                        } else {
                            return child.props.start ? childClass + child.props.start : childClass + start;
                        }
                    })(),
                    ref: (node) => {
            ...
            const childClass =
                Child.props.children.props && Child.props.children.props.className
            ...
            return React.cloneElement(Child, {
                className: `${Child.props.className ? Child.props.className + " " : ""}${
                    isIn ? (Child.props.end ? Child.props.end : end) : Child.props.start ? Child.props.start : start
                }`,
                ref: (node) => {
                    childRef.current = node;
                    if (Child.ref) {
